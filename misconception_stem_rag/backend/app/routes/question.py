"""Routes for question generation and retrieval."""
...
from ..models.question import QuestionModel, QuestionRequest, QuestionResponse
from ..services import cognitive, generation, retrieval, validation
...
async def _load_user_traits(
    ...
    if collection is not None:
        try:
            ...
            logger.warning(
                "Unexpected error when loading traits for user %s", user_id, exc_info=True
            )

    merged_traits = dict(stored_traits)
    if trait_overrides:
        merged_traits.update(trait_overrides)
    return cognitive.derive_traits(merged_traits)
...
async def generate_question(
    ...
    retrieval_result = retrieval.retrieve_context(payload.topic)
    documents = retrieval.flatten_documents(retrieval_result)
    ...
    related_misconceptions = validation.get_related_misconceptions(payload.topic)
    ...
    raw_question = generation.generate_question(
    ...
    try:
        question_model = validation.parse_question_payload(question_payload)
